...
void describe("installLocalExtensionFiles", () => {
	...
	void it("copies the requested package files into the target extension directory", () => {
		...
		const copied = installLocalExtensionFiles({
			sourceRoot,
			targetRoot,
			relativePaths: ["src/extension/index.ts", "src/extension/notify.ts", "package.json", "README.md", "agents/worker.md"],
		});

		assert.deepEqual(copied, ["README.md", "agents/worker.md", "package.json", "src/extension/index.ts", "src/extension/notify.ts"]);
