...
void describe("sandbox install", { skip: !available ? "pi-test-harness not available" : undefined }, () => {
	...
	void it("loads extension after npm pack+install with expected tools", { timeout: 120_000 }, async (t: TestContext) => {
		const originalEnv = {
		...
		const installDir = (result as { installDir?: string; packageDir?: string }).installDir ?? (result as { packageDir?: string }).packageDir;
		if (installDir) {
