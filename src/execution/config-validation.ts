...
const SUPERAGENTS_KEYS = new Set(["commands", "modelTiers", "interceptSkillCommands", "extensions", "tools", "makeSuperpowersSkillsOptInOnly", "superpowersSkills"]);
...
const COMMAND_PRESET_KEYS = new Set(["useBranches", "useSubagents", "useTestDrivenDevelopment", "usePlannotator", "taskScheduling", "reviewCadence", "worktrees"]);
...
function validateCommandPreset(diagnostics: ConfigDiagnostic[], value: unknown, path: string): void {
	...
	if ("reviewCadence" in value && value.reviewCadence !== "per-task" && value.reviewCadence !== "final-only") {
		pushConfigIssue(diagnostics, `${path}.reviewCadence`, 'must be "per-task" or "final-only".');
	}
...
function validateSuperagentsSection(diagnostics: ConfigDiagnostic[], superagents: unknown, options: ConfigValidationOptions): void {
	...
	if ("makeSuperpowersSkillsOptInOnly" in superagents && typeof superagents.makeSuperpowersSkillsOptInOnly !== "boolean") {
		pushConfigIssue(diagnostics, "superagents.makeSuperpowersSkillsOptInOnly", "must be a boolean.");
	}
